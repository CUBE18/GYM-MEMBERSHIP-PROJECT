class Superuser:
    ...
    def addCustomer(cls, customer):
        Superuser.customers[customer.getPhoneNo()] = customer


ob = Superuser('s_1', 'crg')
...
class Customer:
    ...
    def __str__(self):
        return self.getNmae() + " " + self.getPhoneNo() + " " + self.getDuration() + " " + self.getAge() + " " + self.getEmail() + " " + self.getBmi() + " " + self.getGender()


print('  ***** Super user Portal *****  ')
...
while (True):
    ...
    if option == 1:
        ...
        if bmi < '18.5':
            ...
            r = {'Mon': 'Chest', 'Tue': 'Biceps', 'Wed': 'Cardio/Abs', 'Thu': 'Back', 'Fri': 'Triceps', 'sat': 'Legs',
                 'Sun': 'rest'}
        elif bmi < '30':
            r = {'Mon': 'Chest', 'Tue': 'Biceps', 'Wed': 'Abs/Cardio', 'Thu': 'Back', 'Fri': 'Triceps', 'Sat': 'Legs',
                 'sun': 'Cardio'}
        elif bmi >= '30':
        ...
        Superuser.regimen[phoneNo] = r
        Superuser.addCustomer(customer)

    elif option == 2:
        check_phn = input('Enter phone no. of member : ')
        print("Name\tage\tGender\tPhone\tEmail\tBmi\tDuration")
        for cusId in Superuser.customers.keys():
            if cusId == check_phn:
                customer = Superuser.customers[cusId]
        ...
        try:
            ...
            Superuser.customers.pop(check_phn)
        except:
        ...
        for i in Superuser.regimen:
            if i == phn:
                for j in Superuser.regimen[i]:
                    Superuser.regimen[i][j] = input(j + ":")

    elif option == 6:
        check_phn = input('Enter phone no of member : ')
        for i in Superuser.regimen:
            if i == check_phn:
                for key, val in Superuser.regimen[i].items():
                    print(key, ":", val)
        print("\n")
        ...
        Superuser.regimen.pop(check_phn)
        print("\n")
        ...
        for i in Superuser.regimen:
            if i == check_phn:
                ...
                for j in Superuser.regimen[i]:
                    if j == d:
                        Superuser.regimen[i][j] = input('Enter the workout: ')
...
while (True):
    ...
    if option == 1:
        ...
        try:
            for cusId in Superuser.customers.keys():
                if cusId == p:
                    customer = Superuser.customer[cusId]
                    name = customer.getNmae()
                    age = customer.getAge()
                    gender = customer.getGender()
                    phoneNo = customer.getPhoneNo()
                    email = customer.getEmail()
